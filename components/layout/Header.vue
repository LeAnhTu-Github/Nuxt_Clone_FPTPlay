...
const listMenuMobile = [
  { label: 'Đăng nhập/Đăng ký', to: '/login' },
  ...navList,
  ...moreMenu.flat().map(label => ({ label, to: '/' })),
]
...
<template>
  <header
    class="fixed top-0 left-0 right-0 w-full bg-black text-white shadow-[0_1px_0_#222] z-[50] box-border">
    <nav class="my-container mx-auto h-16 sm:h-20 flex items-center justify-between">
      <div class="flex items-center flex-shrink-0 w-full max-w-[150px]">
        <button class="lg:hidden text-white bg-none border-none cursor-pointer p-1 flex items-center mr-2 order-1"
          aria-label="Mở menu" tabindex="0" @click="toggleMobileMenu" @keydown.enter="toggleMobileMenu">
      ...
      <ul class="hidden lg:flex gap-2 xl:gap-4 list-none m-0 pt-4 flex-1">
        <li v-for="nav in navList" :key="nav.label" class="relative flex items-center">
          <NuxtLink :to="nav.to" :class="`
        ...
        <li class="cursor-pointer relative flex items-center">
          <span
            ...
            tabindex="0" aria-haspopup="true" :aria-expanded="isMoreOpen" @click="handleToggleMore"
            @keydown.enter="handleToggleMore" aria-label="Xem thêm">
            Xem thêm ▼
          </span>
          <div v-if="isMoreOpen"
            class="more-modal absolute top-full left-0 mt-2 w-[280px] bg-[#A6A6A6] rounded-xl shadow-lg p-4 sm:p-6 flex gap-4 sm:gap-8 z-[99999]"
            role="menu" tabindex="0" aria-label="Danh sách xem thêm">
            <div class="flex flex-col gap-2 sm:gap-3 flex-1">
              <span v-for="item in moreMenu.map(i => i[0])" :key="item"
                class="text-white text-sm sm:text-base cursor-pointer hover:text-[#fe592a]">
                {{ item }}
              </span>
            </div>
            <div class="flex flex-col gap-2 sm:gap-3 flex-1">
              <span v-for="item in moreMenu.map(i => i[1])" :key="item"
                class="text-white text-sm sm:text-base cursor-pointer hover:text-[#fe592a]">
                {{ item }}
              </span>
            </div>
      ...
      <transition name="fade">
        <div v-if="isMobileMenuOpen" class="fixed inset-0 z-[200] flex" aria-modal="true" role="dialog">
          <div class="absolute inset-0 bg-black bg-opacity-60 transition-opacity duration-300"
            @click="toggleMobileMenu"></div>
          <div
            class="relative h-full w-1/2 max-w-[300px] bg-[#232323] p-4 sm:p-6 flex flex-col gap-4 overflow-y-auto transition-transform duration-300 ease-in-out"
            :class="isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'">
            <ul class="flex flex-col gap-3 list-none m-0 p-0">
              <li v-for="item in listMenuMobile" :key="item.label" class="shrink-0">
                <NuxtLink :to="item.to"
                  class="block text-[#A6A6A6] text-base sm:text-lg font-medium whitespace-nowrap py-2 hover:text-[#fe592a] transition"
      ...
      <div class="flex items-center gap-1 sm:gap-2 ml-2 shrink-0">
        <button class="hidden lg:flex text-white bg-none border-none cursor-pointer p-1 items-center"
          aria-label="Tìm kiếm" tabindex="0">
        ...
        <button class="hidden lg:flex text-white bg-none border-none cursor-pointer p-1 items-center"
          aria-label="Thông báo" tabindex="0">
          <svg width="20" height="20" class="sm:w-[22px] sm:h-[22px]" fill="none" stroke="currentColor">
        ...
        <NuxtLink to="/login"
          class="hidden lg:flex text-white text-sm sm:text-base no-underline opacity-85 hover:opacity-100 transition-opacity duration-200  rounded-lg py-1 px-3 sm:py-2 sm:px-5 mr-1 sm:mr-2">
          Đăng nhập
        </NuxtLink>
      </div>
    </nav>
